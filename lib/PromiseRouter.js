...
function makeExpressHandler(appId, promiseHandler) {
  ...
  return function (req, res, next) {
    try {
      var _url = maskSensitiveUrl(req);
      var body = maskSensitiveBody(req);
      var stringifiedBody = JSON.stringify(body, null, 2);
      _logger2.default.verbose('REQUEST for [' + req.method + '] ' + _url + ': ' + stringifiedBody, {
        method: req.method,
        url: _url,
        headers: req.headers,
        body: body
      ...
      promiseHandler(req).then(function (result) {
        ...
        _logger2.default.verbose('RESPONSE from [' + req.method + '] ' + _url + ': ' + stringifiedResponse, { result: result });
...
function maskSensitiveUrl(req) {
  ...
  if (shouldMaskUrl) {
    var password = _url3.default.parse(req.originalUrl, true).query.password;
    if (password) {
