...
function transformQueryKeyValue(className, key, value, schema, extraOut) {
  switch (key) {
    ...
    case '$or':
      return { key: '$or', value: value.map(function (subQuery) {
          return transformWhere(className, subQuery, schema, extraOut);
        }) };
    case '$and':
  ...
  var transformConstraintValue = transformConstraint(value, expectedTypeIsArray, extraOut);
  if (transformConstraintValue !== CannotTransform) {
    return { key: key, value: transformConstraintValue };
  }
...
function transformWhere(className, restWhere, schema, extraOut) {
  ...
  for (var restKey in restWhere) {
    var out = transformQueryKeyValue(className, restKey, restWhere[restKey], schema, extraOut);
