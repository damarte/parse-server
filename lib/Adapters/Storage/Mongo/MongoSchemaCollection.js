  ...
  _createClass(MongoSchemaCollection, [{
    ...
    value: function _fetchAllSchemasFrom_SCHEMA() {
      ...
      if (this._cache && this._cacheTime && new Date().getTime() - this._cacheTime.getTime() <= 600000) {
        ...
        return this._collection._rawFind({}).then(function (schemas) {
          _this._cacheTime = new Date();
          _this._cache = schemas.map(mongoSchemaToParseSchema);
          return _this._cache;
        });
      }
    }
    ...
    value: function addFieldIfNotExists(className, fieldName, type) {
      ...
      return this._fechOneSchemaFrom_SCHEMA(className).then(function (schema) {
        ...
        return _this2.upsertSchema(className, _defineProperty({}, fieldName, { '$exists': false }), { '$set': _defineProperty({}, fieldName, parseFieldTypeToMongoFieldType(type)) });
      });
    }
  }]);
