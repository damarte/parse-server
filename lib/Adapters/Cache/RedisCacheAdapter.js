'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.RedisCacheAdapter = undefined;

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

var _redis = require('redis');

var _redis2 = _interopRequireDefault(_redis);

var _logger = require('../../logger');

var _logger2 = _interopRequireDefault(_logger);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function debug() {
  _logger2.default.debug.apply(_logger2.default, ['RedisCacheAdapter'].concat(Array.prototype.slice.call(arguments)));
}

var RedisCacheAdapter = exports.RedisCacheAdapter = function () {
  function RedisCacheAdapter(ctx) {
    _classCallCheck(this, RedisCacheAdapter);

    this.client = _redis2.default.createClient(ctx);
    this.p = Promise.resolve();
  }

  _createClass(RedisCacheAdapter, [{
    key: 'get',
    value: function get(key) {
      var _this = this;

      debug('get', key);
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          _this.client.get(key, function (err, res) {
            debug('-> get', key, res);
            if (!res) {
              return resolve(null);
            }
            resolve(JSON.parse(res));
          });
        });
      });
      return this.p;
    }
  }, {
    key: 'getMany',
    value: function getMany(keys) {
      var _this2 = this;

      debug('getMany', keys);
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          _this2.client.multi(keys.map(function (key) {
            return ['get', key];
          })).exec(function (err, res) {
            debug('-> getMany', keys, res);
            if (!res) {
              return resolve(null);
            }
            resolve(res.map(function (resItem) {
              return resItem ? JSON.parse(resItem) : null;
            }));
          });
        });
      });
      return this.p;
    }
  }, {
    key: 'put',
    value: function put(key, value, ttl) {
      var _this3 = this;

      if (value == undefined || value == 'undefined') {
        return del(key);
      }
      var value = JSON.stringify(value);
      debug('put', key, value, ttl);
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          if (ttl) {
            _this3.client.set(key, value, 'px', ttl, function (err, res) {
              resolve();
            });
          } else {
            _this3.client.set(key, value, function (err, res) {
              resolve();
            });
          }
        });
      });
      return this.p;
    }
  }, {
    key: 'putMany',
    value: function putMany(items, ttl) {
      var _this4 = this;

      var values = {};
      var command = [];
      for (var key in items) {
        values[key] = JSON.stringify(items[key]);
        command.push(['set', key, values[key], 'px', ttl]);
      }
      debug('putMany', values, ttl);
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          _this4.client.multi(command).exec(function (err, res) {
            resolve();
          });
        });
      });
      return this.p;
    }
  }, {
    key: 'del',
    value: function del(key) {
      var _this5 = this;

      debug('del', key);
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          _this5.client.del(key, function (err, res) {
            resolve();
          });
        });
      });
      return this.p;
    }
  }, {
    key: 'clear',
    value: function clear() {
      var _this6 = this;

      debug('clear');
      this.p = this.p.then(function () {
        return new Promise(function (resolve, _) {
          _this6.client.flushall(function (err, res) {
            resolve();
          });
        });
      });
      return this.p;
    }
  }]);

  return RedisCacheAdapter;
}();

exports.default = RedisCacheAdapter;