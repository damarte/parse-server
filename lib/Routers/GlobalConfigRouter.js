...
var GlobalConfigRouter = exports.GlobalConfigRouter = function (_PromiseRouter) {
  ...
  _createClass(GlobalConfigRouter, [{
    ...
    value: function getGlobalConfig(req) {
      var database = req.config.database.WithoutValidation();
      return database.find('_GlobalConfig', { objectId: 1 }, { limit: 1 }).then(function (results) {
        if (results.length != 1) {
    ...
    value: function updateGlobalConfig(req) {
      ...
      var database = req.config.database.WithoutValidation();
      return database.update('_GlobalConfig', { objectId: 1 }, update, { upsert: true }).then(function () {
