...
function handleImportRelation(req, res) {
  ...
  function getOneSchema() {
    ...
    return req.config.database.loadSchema({ clearCache: true }).then(function (schemaController) {
      ...
      if (error === undefined) {
        return Promise.reject(new _node2.default.Error(_node2.default.Error.INVALID_CLASS_NAME, 'Class ' + className + ' does not exist.'));
      } else {
        return Promise.reject(new _node2.default.Error(_node2.default.Error.INTERNAL_SERVER_ERROR, 'Database adapter error.'));
      }
    });
  }

  function importRestObject(restObject) {
    return _rest2.default.update(req.config, req.auth, req.params.className, { objectId: restObject.owningId }, _defineProperty({}, req.params.relationName, {
      "__op": "AddRelation",
      "objects": [{ "__type": "Pointer", "className": targetClass, "objectId": restObject.relatedId }]
    }), req.info.clientSDK).catch(function (error) {
      if (error.code === _node2.default.Error.OBJECT_NOT_FOUND) {
        return Promise.reject(new _node2.default.Error(_node2.default.Error.OBJECT_NOT_FOUND, 'Object not found'));
      } else {
